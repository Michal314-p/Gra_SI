                ...
                for (var i = 0; i < 5; i++) {
                    {
                        if (tablica[j][i] === 0) {
                            tablica[j][i] = player;
                            var aktualna_wartosc = this.mini_maks(1, tablica, 0, -2147483648, 2147483647);
                            tablica[j][i] = 0;
    ...
    mini_maks = function (gracz, tablica, aktualna_depth, alpha, beta) {
                    ...
                    for (var i = 0; i < 5; i++) {
                        {
                            if (tablica[j][i] === 0) {
                                tablica[j][i] = gracz;
                                najlepszy = Math.max(najlepszy, this.mini_maks(1, tablica, aktualna_depth + 1, alpha, beta));
                                tablica[j][i] = 0;
                                if (najlepszy > alpha) {
                                    alpha = najlepszy;
                                }
                    ...
                    for (var i = 0; i < 5; i++) {
                        {
                            if (tablica[j][i] === 0) {
                                tablica[j][i] = gracz;
                                najlepszy = Math.min(najlepszy, this.mini_maks(2, tablica, aktualna_depth + 1, alpha, beta));
                                tablica[j][i] = 0;
                                if (najlepszy < beta) {
                                    beta = najlepszy;
                                }
